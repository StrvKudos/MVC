...
function getThumbnailsByVehicle($invId){
    $db = phpmotorsConnect();
    $sql = 'SELECT imgPath FROM images WHERE images.imgPrimary = 0 AND images.imgPath LIKE "%-tn%" AND invId IN (SELECT invId FROM inventory WHERE invId = :invId)';
    $stmt = $db->prepare($sql);
    $stmt->bindValue(':invId', $invId, PDO::PARAM_STR);
    $stmt->execute();
    $thumbnailDetails = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $stmt->closeCursor();
