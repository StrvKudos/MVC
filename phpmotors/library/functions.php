...
function navList($classifications) {
    ...
    foreach ($classifications as $classification) {
        $navList .= "<li><a href='/phpmotors/vehicle/?action=classification&classificationName=".urlencode($classification['classificationName'])."' title='View our $classification[classificationName] product line'>$classification[classificationName]</a></li>";
}
    $navList .= "</ul>";
    return $navList;
    }
...
function buildClassificationList($classifications){ 
    ...
    function buildVehiclesDisplay($vehicles){
        ...
        foreach ($vehicles as $vehicle) {
         ...
         $dv .= '<li>';
         $dv .= "<a href='/phpmotors/vehicle/?action=getVehicleDetail&valueId=$vehicle[invId]'><img src='..$vehicle[invThumbnail]' alt='Image of $vehicle[invMake] $vehicle[invModel] on phpmotors.com'></a>";
         $dv .= '<hr>';
         $dv .= "<h2><a href='/phpmotors/vehicle/?action=getVehicleDetail&valueId=$vehicle[invId]'>$vehicle[invMake] $vehicle[invModel]</a></h2>";
...
function processImage($dir, $filename) {
   ...
   function buildReviewsDisplay($reviews){
    ...
    if(isset($_SESSION['clientData']['clientFirstname'])){
      ...
      $dv .= '<label for="reviewText"><br>Review:</label><br>';
      $dv .= '<input type="text" id="reviewText" name="reviewText" required><br><br>';
